...
def get_filehash(file):
    ...
    # 해시알고리즘 적용전 파일내용을 청크chunk 단위로 읽음 - 대용량파일
    # 청크chunk: 큰 데이터를 처리하기 편하게 일정 크기로 잘라서 처리하는 데이터 조각
    # 청크chunk의 장점 - 메모리 절약, 스트리밍 처리, 입출력 효율
    # lambda: f.read(8192), b"" ->[청크][청크][청크][청크][청크][청크][청크]
    with open(file, 'rb') as f:
        # for chunk in iter(lambda: f.read(8192), b""):     #  8KB
        for chunk in iter(lambda: f.read(95536), b""):      # 64KB
            md5.update(chunk)
            sha1.update(chunk)
            sha256.update(chunk)
...
if __name__ == '__main__':
    ...
    md5hashed, sha1hased, sha256hashed = get_filehash(file_path)
    print(md5hashed)
    print(sha1hased)
    print(sha256hashed)
