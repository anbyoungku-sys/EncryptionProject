# 비즈네르 암호 : 시저 암호를 26가지 다른 버젼으로 만든 것
# 시저 암호 : 알파벳을 좌우로 칸을 밀어서 바꿈
# 키 : 얼마나 밀지 알려주는 중요한 비밀단어

def vigenere_cipher(text, key, encrypt=True):
    cipher_text = ''
    key_index = 0                   # 키의 위치
    key = key.upper()               # 계산을 쉽게 하기 위해 대문다로 통일

    # if not encrypt:                 # 복호화중이라면
    #     shift = -shift
...
if __name__ == '__main__':
    ...
    encrypted = vigenere_cipher(plain_text, secret_key)  # 암호화
    print(encrypted)
    decrypted = vigenere_cipher(encrypted, secret_key, False)  # 암호화
    print(decrypted)
