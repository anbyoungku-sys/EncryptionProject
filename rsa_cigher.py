...
def rsa_encrypt(text, key):
    # 키 문자열을 RSA 공개키 객체로 변화
    recipient_key = RSA.import_key(key)
    # RSA-OAEP 방식으로 패딩 처리 - 무작위성 기반
    # PKCS : 공개키 암호 표준
    # OAEP : 암호화전 평문을 무작위화 + 섞고 패딩하는 방법->같은 메세지라도 암호화시 매번 다른 암호문 생성
    rsa_cipher = PKCS1_OAEP.new(recipient_key)
    # 메세지 utf-8인코딩 후 암호와 수행
...
from aes_cipher import plaintext, encrypted

print(" RSA 키 쌍을 생성합니다. ")
